...
d = read.table(file = "out_SZA_23092015.csv", header = TRUE, sep  = ",")
d = tbl_df(d)

mozart.data = d %>% filter(Mechanism == "MOZART") %>% select(O3, NOx.Emissions, Solar.Zenith.Angle)
mozart.data = mozart.data %>% filter(NOx.Emissions <= 7.5e9)
mozart.data = mozart.data %>% mutate(Scaled.Solar.Zenith.Angle = (Solar.Zenith.Angle - min(Solar.Zenith.Angle))/(max(Solar.Zenith.Angle) - min(Solar.Zenith.Angle)), Scaled.NOx.Emissions = (NOx.Emissions - min(NOx.Emissions))/(max(NOx.Emissions) - min(NOx.Emissions)))
...
mozart.fld = with(mozart.data, interp(x = Scaled.Solar.Zenith.Angle, y = Scaled.NOx.Emissions, z = O3, duplicate = "strip"))
mozart.df = melt(mozart.fld$z, na.rm = TRUE)
names(mozart.df) = c("x", "y", "O3")
mozart.df$Solar.Zenith.Angle = mozart.fld$x[mozart.df$x]
...
relative.humidity.break.points = seq(0, 1, 0.2)
relative.humidity.labels = get.labels(relative.humidity.break.points, mozart.data$Solar.Zenith.Angle, digits = 2)
...
p = ggplot(df, aes(x = Solar.Zenith.Angle, y = NOx.Emissions, z = O3))
p = p + stat_contour(aes(colour = ..level..)) 
p = p + facet_wrap(~ Mechanism) 
p = p + theme_tufte() 
p = p + theme(axis.line = element_line(colour = "black")) 
p = p + theme(strip.text = element_text(face = "bold")) 
p = p + xlab("Solar Zenith Angle (oN)") 
...
p = p + scale_x_continuous(breaks = relative.humidity.break.points, labels = relative.humidity.labels)
p = p + scale_y_continuous(breaks = NOx.Emissions.break.points, labels = NOx.Emissions.labels)

CairoPDF(file = "plot_solar_zenith_angle.pdf", width = 10, height = 7)
