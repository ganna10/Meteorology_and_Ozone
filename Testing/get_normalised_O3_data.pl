#! /usr/bin/env perl
# Get O3 budget normalised by total NMVOC emissions. Mechanism is ARGUMENT
...
#my $mecca = MECCA->new("$base/$dir");
    ...
    my $voc_emissions = get_all_emissions($mecca, $kpp); #sum first day
    ...
    foreach (keys %production_reaction_rate) {
        ...
        my $sum = $reshape->sum;
        $production_reaction_rate{$_} = $sum / $voc_emissions; #normalise by VOC emissions
    }

    foreach my $reaction (sort keys %production_reaction_rate) {
        print $out "$mechanism,$h2o2,$hno3,$reaction,$production_reaction_rate{$reaction}\n";
    }
#}
