...
norm.list.td <- lapply(mechanisms, get_budget_data, Species = "HOx", Run.Label = "TD", Date = "23022016")
norm.td.df <- do.call("rbind", norm.list.td)
tbl_df(norm.td.df)

# temperature independent normalising data
norm.list.ti <- lapply(mechanisms, get_budget_data, Species = "HOx", Run.Label = "TI", Date = "23022016")
...
norm.data <- norm.df %>%
  ...
  group_by(Run, Mechanism, Temperature.C, NOx.Condition, Category) %>%
...
hox.prod <- norm.data %>%
  ...
  summarise(VOC.initial.losses = sum(Rate)) # to use in normalising_rates fn
tbl_df(hox.prod)
...
d <- df %>%
  ...
  mutate(Rate = normalising_rates(Rate, Normalising.df = hox.prod, mechanism = Mechanism, temperature = Temperature.C, run = Run, NOx.condition = NOx.Condition))
...
d$Category <- factor(d$Category, levels = c("HO2", "ARO2", "RO2", "Inorganic", "Other Organic", "RO2NO2"))

my.colours = c("Inorganic" = "#2b9eb3", "Other Organic" = "#000000", "RO2" = "#0e5c28", "ARO2" = "#f9c500", "RO2NO2" = "#ef6638", "HO2" = "#6c254f")
...
CairoPDF(file = "TD_Ox_normalised_HOx.pdf", width = 10, height = 7)
print(p)
dev.off()
