...
# Version 1: Jane Coates 31/7/2016 correcting plotting error

setwd("~/Documents//Analysis/2015_Meteorology_and_Ozone/Budgets/")
data <- read.csv("Normalised_Ox_budget.csv")
tbl_df(data)
d <- data %>%
...
td.plot <- td.plot + scale_y_continuous(limits = c(-1, 3), breaks = seq(-1, 3, 1), expand = c(0, 0))
td.plot <- td.plot + scale_fill_manual(values = my.colours)
...
ti.plot <- ti.plot + scale_y_continuous(limits = c(-1, 3), breaks = seq(-1, 3, 1), expand = c(0, 0))
...
CairoPDF(file = "Normalised_Ox_Budget_categories.pdf", width = 10, height = 7)
...
print.data.frame(diff.df %>%
  filter(Category == "ARO2"| Category == "RO2NO2") %>%
  spread(Run, Diff))
