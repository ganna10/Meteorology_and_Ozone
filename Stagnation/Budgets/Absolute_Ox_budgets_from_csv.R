...
# Version 1: Jane Coates 31/7/2016 corrected plotting error, adding net budget
...
td.plot <- td.plot + scale_y_continuous(limits = c(-5e8, 2.5e9), breaks = seq(0, 2e9, 1e9), expand = c(0, 0))
td.plot <- td.plot + scale_fill_manual(values = my.colours)
td.plot <- td.plot + theme(panel.margin = unit(4, "mm"))
td.plot <- td.plot + xlab(expression(bold(paste("Temperature (", degree, "C)"))))
td.plot <- td.plot + ylab("Ox Budget (molecules (Ox) cm-3)")
...
ti.plot <- ti.plot + scale_y_continuous(limits = c(-5e8, 2.5e9), breaks = seq(0, 2e9, 1e9), expand = c(0, 0))
...
CairoPDF(file = "Absolute_Ox_Budget_categories-No_mixing.pdf", width = 10, height = 7)
