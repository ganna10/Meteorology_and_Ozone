...
my $date = $ARGV[0];
my $run = $ARGV[1];
my @variables = qw( NOx.Emissions Temperature NOx.Condition O3 H2O2 OH HO2 HOx HCHO RO2NO2 RONO2 HNO3 PAN Ketones Aldehydes MDA8 );

my $base = cwd();
opendir DIR, "$base/$run" or die $!;
my @files = grep { $_ =~ /_check_Temperature_${date}\.txt$/ } readdir DIR;
close DIR;

my $out_file = "${run}_data_${date}.csv";
...
foreach my $checkfile (@files) { 
    ...
    my ($nox_emission, $temperature, $NOx_condition, $O3, $H2O2, $OH, $HO2, $HOx, $HCHO, $RO2NO2, $RONO2, $HNO3, $PAN, $Ketones, $Aldehydes, $MDA8) = extract_data($mechanism, $checkfile, \@variables);
    
    for (0..$#$nox_emission) {
        $Ketones->[$_] = 0 if ($mechanism eq "CB05");
        print $out "$mechanism,$run,$nox_emission->[$_],$temperature->[$_],$NOx_condition->[$_],$O3->[$_],$H2O2->[$_],$OH->[$_],$HO2->[$_],$HOx->[$_],$HCHO->[$_],$RO2NO2->[$_],$RONO2->[$_],$HNO3->[$_],$PAN->[$_],$Ketones->[$_],$Aldehydes->[$_],$MDA8->[$_]\n";
    }
}
...
sub extract_data {
    ...
    open my $in, '<:encoding(utf-8)', "$run/$file" or die $!;
    local $/ = undef; #slurp mode
    my $all_lines = <$in>;
    close $in; 
    my @lines = split /\n/, $all_lines;

    my (@nox_emissions, @temperature, @NOx_condition, @O3, @H2O2, @OH, @HO2, @HNO3, @HONO, @HOx, @HCHO, @RO2NO2, @RONO2, @PAN, @Ketones, @Aldehydes, @MDA8);
    foreach my $variable (@$variables) { 
        if ($variable eq "NOx.Emissions") {
            ...
            foreach my $line (@lines) { 
                if ($line =~ /^T_/) {
                    (my $temperature = $line) =~ s/^T_(.*?)_(.*?)$/$1/;
                    (my $nox_condition = $line) =~ s/^T_(.*?)_(.*?)$/$2/;
                    push @temperature, $temperature;
                    if ($nox_condition =~ /Hig/) {
                        $nox_condition = "High-NOx";
                    } elsif ($nox_condition =~ /Low/) {
                        $nox_condition = "Low-NOx";
                    } elsif ($nox_condition =~ /Max/) {
                        $nox_condition = "Maximal-O3";
                    }
                    push @NOx_condition, $nox_condition;
                }
            }
    ...
    return \@nox_emissions, \@temperature, \@NOx_condition, \@O3, \@H2O2, \@OH, \@HO2, \@HOx, \@HCHO, \@RO2NO2, \@RONO2, \@HNO3, \@PAN, \@Ketones, \@Aldehydes, \@MDA8;
}
