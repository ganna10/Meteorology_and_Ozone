...
test = h2o2.hno3 %>% filter(Mechanism == "CB05" & Temperature == "293")
test$H2O2.HNO3.Ratio

my.colours = c("MCMv3.2" = "#000000", "CB05" = "#0e5c28", "RADM2" = "#f9c500", "MOZART-4" = "#6c254f", "CRIv2" = "#ef6638")

plot.lines = function () {
    list(   geom_point(),
            facet_wrap( ~ NOx.Emissions),
...
p = ggplot(h2o2.hno3, aes(x = Temperature, y = H2O2.HNO3.Ratio, colour = Mechanism))
p = p + ylab("H2O2/HNO3 in ppbv/ppbv")
#p = p + scale_y_continuous(limits = c(0, 250), breaks = seq(0, 250, 25))
p = p + plot.lines()

CairoPDF(file = "H2O2_HNO3_ratios_facet_T.pdf")
print(p)
dev.off()
