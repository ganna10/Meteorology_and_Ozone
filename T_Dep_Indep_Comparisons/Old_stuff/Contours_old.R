...
contour_mechanism_data_frame = function (mechanism, dataframe) {
  ...
  fld = with(data, interp(x = Scaled.Temperature, y = Scaled.NOx.Emissions, z = Acetone))
  df = melt(fld$z, na.rm = TRUE)
  names(df) = c("x", "y", "Acetone")
...
get_contour_data = function (run) {
  filename = paste0("Temperature_", run, "_Acetone_data.csv")
  d = read.csv(filename)
...
mechanisms = c("RADM2", "MCMv3.2", "MOZART-4", "CRIv2")
...
#labels
d = read.csv(file = "Temperature_Dependent_data.csv") %>% filter(Mechanism == "RADM2") %>% mutate(Temperature.C = Temperature - 273)
...
p = p + stat_contour(aes(x = Temperature, z = Acetone, colour = ..level..)) 
...
direct.label(p)
# CairoPDF(file = "Acetone_contours.pdf", width = 7, height = 10)
