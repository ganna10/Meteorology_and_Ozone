...
d = read.table(file = "out_Relative_Humidity_22092015.csv", header = TRUE, sep  = ",")
d = tbl_df(d)

mozart.data = d %>% filter(Mechanism == "MOZART") %>% select(O3, NOx.Emissions, Relative.Humidity)
mozart.data = mozart.data %>% filter(NOx.Emissions <= 7.5e9)
mozart.data = mozart.data %>% mutate(Scaled.Relative.Humidity = (Relative.Humidity - min(Relative.Humidity))/(max(Relative.Humidity) - min(Relative.Humidity)), Scaled.NOx.Emissions = (NOx.Emissions - min(NOx.Emissions))/(max(NOx.Emissions) - min(NOx.Emissions)))
...
mozart.fld = with(mozart.data, interp(x = Scaled.Relative.Humidity, y = Scaled.NOx.Emissions, z = O3, duplicate = "strip"))
mozart.df = melt(mozart.fld$z, na.rm = TRUE)
names(mozart.df) = c("x", "y", "O3")
mozart.df$Relative.Humidity = mozart.fld$x[mozart.df$x]
...
relative.humidity.labels = get.labels(relative.humidity.break.points, mozart.data$Relative.Humidity, digits = 2)
...
p = ggplot(df, aes(x = Relative.Humidity, y = NOx.Emissions, z = O3))
p = p + stat_contour(aes(colour = ..level..)) 
p = p + facet_wrap(~ Mechanism) 
p = p + theme_tufte() 
p = p + theme(axis.line = element_line(colour = "black")) 
p = p + theme(strip.text = element_text(face = "bold")) 
p = p + xlab("Relative Humidity") 
...
CairoPDF(file = "plot_relative_humidity.pdf", width = 10, height = 7)
